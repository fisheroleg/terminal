...
class Dashboard extends CI_Controller {
    ...
    function go()
    {
	$matrix = json_decode($this->input->post('data'),FALSE);
	foreach($matrix as $i=>$col)
		foreach($col as $j=>$row)
			$matrix[$i][$j] = intval($row);
	//die(var_dump($matrix));
	$this->load->model('content_model');
		$this->content_model->init($matrix);
		
		$this->content_model->multiple();
		$this->content_model->output();
    }
