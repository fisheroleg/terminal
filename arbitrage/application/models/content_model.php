...
Class Content_model extends CI_Model
{
    ...
    function multiple()
    {
        $this->mult_matrix = array();   //2
        foreach ($this->matrix as $i=>$col) //3
        {
            $c = min($col);//4
            foreach ($col as $j=>$row) //5
            {
                $this->mult_matrix[$i][$j] = $this->matrix[$i][$j] * $c;//6
            }
        }
        
        $this->mult_matrix = $this->transpose($this->mult_matrix);
    }
    
    function get()
    {
        return $this->mult_matrix;//7
    }
    
    function output()
    {
        echo '<table>';
        foreach ($this->mult_matrix as $col)
        {
            echo '<tr>';
            foreach ($col as $row)
            {
